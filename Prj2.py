...
car_data = {}  # Dữ liệu về các xe trong bãi
...
def archive_image(file_path):
    if os.path.exists(file_path):
        archive_path = os.path.join(ARCHIVE_PATH, os.path.basename(file_path))
        os.rename(file_path, archive_path)
        threading.Timer(86400, os.remove, args=[archive_path]).start()
