...
def capture_and_recognize(slot):
    ...
    if ret:
        # Lưu ảnh chụp
        cv2.imwrite(filename, frame)
        print(f"Ảnh chụp được lưu tại: {filename}")
    else:
        ...
        for row in range(8):
            for col in range(8):
                if parking_lot[row] & (1 << (7 - col)):  # Tìm vị trí trống
                    ...
                    lcd_clear()
                    lcd_display_string(f"Xe vao: ", 1)
                    for i in range(LCD_WIDTH - len(f"{plate_number}")):
                        lcd_clear()
                        lcd_display_string(" " * i + f"{plate_number}", 2)
                        time.sleep(0.25)
                    time.sleep(3)
                    available_slots -= 1
                    open_gate()
...
def main():
    ...
    update_lcd(total_slots)  # Cập nhật LCD
